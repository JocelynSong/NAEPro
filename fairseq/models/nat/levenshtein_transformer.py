...
class LevenshteinTransformerModel(FairseqNATModel):
    ...
    def forward(
        ...
        )  # predicted insert probability: [B, L, 256] (each token predict a position between 0~256)
        ...
        # make online prediction : [Batch, Length]
...
class LevenshteinTransformerDecoder(FairseqNATDecoder):
    ...
    def forward_mask_ins(self, normalize, encoder_out, prev_output_tokens, **unused):
        ...
        decoder_out = F.linear(features_cat, self.embed_mask_ins.weight)  # [batch, length-1, 256(max length)]
        if normalize:
