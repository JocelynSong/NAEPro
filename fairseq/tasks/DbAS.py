...
class SampleDataset(FairseqDataset):
    ...
    def __init__(self, samples, fits, dictionary, append_eos=True, reverse_order=False):
        self.tokens_list = []
        self.lines = []
        self.sizes = []
...
class FitnessWeightDataset(BaseWrapperDataset):
    def __init__(self, dataset, reduce=False):
        super().__init__(dataset)
        self.reduce = reduce

    def __getitem__(self, index):
        item = self.dataset.get_fitness_value(index)
        return item
...
@dataclass
class DBASConfig(FairseqDataclass):
    data: str = field(
        default=MISSING,
        metadata={
    ...
    number_round_optimization: int = field(
        default=20,
        metadata={"help": "number of dbas round optimization"}
    )
...
@register_task("DbAS", dataclass=DBASConfig)
class DBASTask(FairseqTask):

    cfg: DBASConfig

    """Task for training masked language models (e.g., BERT, RoBERTa)."""

    def __init__(self, cfg: DBASConfig, dictionary, starting_sequence):
        super().__init__(cfg)
    ...
    @classmethod
    def setup_task(cls, cfg: DBASConfig, **kwargs):
    ...
    def normalize_fitness_values(self, fits):
        fits = 1.0 / (1.0 + np.exp(-np.array(fits)))
        return fits
    ...
    def load_round_dataset(self, split, model):
        ...
        indexes = np.argsort(-np.array(fitness_values))[: int(self.number_round_samples/3)]
        final_samples = [samples[index] for index in indexes]
        final_fits = [fitness_values[index] for index in indexes]
        final_fits = self.normalize_fitness_values(final_fits)

        dataset = SampleDataset(final_samples, final_fits, self.dictionary)
        fits_dataset = FitnessWeightDataset(dataset, reduce=False)
    ...
    def build_dataset_for_inference(self, src_tokens, src_lengths, sort=True):
        fits = [1.0]
        src_dataset = SampleDataset(src_tokens, fits, self.dictionary)
